...
1. **증감 연산자** (`a++` vs `++a`) → 거의 매회 출제
2. **switch-case에서 break 누락** → fall-through 주의
3. **상속 + 오버라이딩 + 업캐스팅** → 실행 결과 추적
4. **문자열 비교는 equals()** → `==` 쓰면 틀림
5. **정수 나누기 소수점 버림** → `10 / 3 = 3`
6. **배열 인덱스 0부터 시작** → 범위 초과 주의
7. **생성자 실행 순서** → 부모 생성자 먼저, 그 다음 자식
8. **static 변수 공유** → 모든 객체가 같은 값 참조
